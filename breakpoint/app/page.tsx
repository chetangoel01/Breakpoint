...
export default function FatigueTracker() {
  ...
  const [completedSessions, setCompletedSessions] = useState<
    Array<{ duration: number; breakInterval: number }>
  >([]);
  ...
  const [fatigueStatus, setFatigueStatus] = useState<"alert" | "drowsy">(
    "alert",
  );
  ...
  useEffect(() => {
    ...
    const getSensitivityMultiplier = () => {
      switch (settings.fatigueDetection.sensitivityLevel) {
        case "high":
          return fatigueStatus === "drowsy" ? 1.5 : 1;
        case "medium":
          return fatigueStatus === "drowsy" ? 1.3 : 1;
        case "low":
          return fatigueStatus === "drowsy" ? 1.1 : 1;
        default:
          return 1;
      }
    };
  ...
  useEffect(() => {
    if (window.electronAPI && window.electronAPI.updateFatigueDetection) {
      window.electronAPI.updateFatigueDetection(
  ...
  if (isMinimized) {
    return (
      <div className="min-h-screen glass--effect flex items-center justify-center transition-all duration-300">
        <div
          className={`w-full h-full flex flex-col items-center justify-between p-2 glass--effect rounded-xl border-2 transition-all duration-500 relative`}
        >
          {/* Timer and Status */}
        ...
        <div className="flex-1 scale-padding space-y-8 flex flex-col justify-center">
          <div className="flex items-center scale-gap">
            <div className="flex-1 text-center scale-space-y">
              ...
              {settings.fatigueDetection.enabled &&
                ...
                fatigueStatus === "drowsy" &&
                drowsyDuration >
                  settings.fatigueDetection.drowsyThreshold * 0.7 && (
                  <div className="flex items-center justify-center gap-2 px-4 py-2 bg-orange-100 dark:bg-orange-900/20 rounded-full border border-orange-200 dark:border-orange-800">
                    <span className="text-orange-600 dark:text-orange-400 text-sm font-medium">
                      {drowsyDuration >=
                      settings.fatigueDetection.drowsyThreshold
          ...
          <div className="flex items-center justify-center scale-gap py-2 px-4 rounded-2xl">
            <div className="text-center">
              ...
              <div className="scale-text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wide">
                Sessions
              </div>
            </div>
            <div className="text-center">
